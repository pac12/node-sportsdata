  ...
  describe '#getPlayByPlay()', ->
    ...
    before ->
      scope = nock('http://api.sportsdatallc.org')
        .get('/ncaafb-t1/2013/REG/1/MICH/MSU/pbp.xml?api_key=bad-key')
        .replyWithFile(403, __dirname + '/replies/api-key-error.txt')
        .get("/ncaafb-t1/#{new Date().getFullYear()}/REG/1/MICH/MSU/pbp.xml?api_key=api-key")
        .replyWithFile(200, __dirname + '/replies/ncaafb-pbp-200.txt')
        .get("/ncaafb-t1/2013/REG/1/MICH/MSU/pbp.xml?api_key=api-key")
        .replyWithFile(200, __dirname + '/replies/ncaafb-pbp-200.txt')
        .get("/ncaafb-t1/2013/REG/1/MICH/OSU/pbp.xml?api_key=api-key")
        .reply(404)

    it 'should be a function', ->
      ncaafb.getPlayByPlay.should.be.a('function')

    it 'should pass error and no result with missing away param', (done) ->
      ncaafb.getPlayByPlay (err, result) ->
        err.should.match /property "away" does not exist/
        should.not.exist result
        done()

    it 'should pass error and no result with missing home param', (done) ->
      ncaafb.getPlayByPlay {away: 'MICH'}, (err, result) ->
        err.should.match /property "home" does not exist/
        should.not.exist result
        done()

    it 'should pass error and no result with bad api key', (done) ->
      badNcaafb.getPlayByPlay {year: 2013, away: 'MICH', home: 'MSU'}, (err, result) ->
        err.should.match /HTTP 403/
        should.not.exist result
        done()

    it 'should default to current year, REG, and week 1', (done) ->
      ncaafb.getPlayByPlay {away: 'MICH', home: 'MSU'}, (err, result) ->
        should.not.exist err
        result.should.be.a 'object'
        done()

    it 'should pass no error and schedule as result on 200', (done) ->
      ncaafb.getPlayByPlay {year: 2013, away: 'MICH', home: 'MSU'}, (err, result) ->
        should.not.exist err
        result.should.be.a 'object'
    ...
    it 'should pass error and no result with bad param', (done) ->
      ncaafb.getPlayByPlay {year: 2013, away: 'MICH', home: 'OSU'}, (err, result) ->
